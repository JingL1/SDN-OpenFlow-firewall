...
def parse_config(filename):
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            if len(cleanline.split(',')) != 9:
                raise TypeError("There are only %i parts to the line \"%s\"; there must be 9."
                                % (len(cleanline.split(',')), line))

            (rulenum, macaddr_src, macaddr_dst, ipaddr_src, ipaddr_dst, 
             port_src, port_dst, protocol, ipproto) = cleanline.split(',')
            
            
            if (macaddr_src != '-' and
                None == re.match("[0-9a-f]{2}([:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", 
                                 macaddr_src.lower())):
                raise TypeError("Source MAC Address for rulenum %s is invalid" % rulenum)
            if (macaddr_dst != '-' and
                None == re.match("[0-9a-f]{2}([:])[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", 
                                 macaddr_dst.lower())):
                raise TypeError("Destination MAC Address for rulenum %s is invalid" % rulenum)
            
            if (ipaddr_src != '-' and not valid_ip(ipaddr_src)):
                raise TypeError("Source IP Address for rulenum %s is invalid" % rulenum)                
            if (ipaddr_dst != '-' and not valid_ip(ipaddr_dst)):
                raise TypeError("Destination IP Address for rulenum %s is invalid" % rulenum)                
                
            if (port_src != '-' and (int(port_src) > 65535 or int(port_src) < 1)):
                raise TypeError("Source Port for rulenum %s is invalid" % rulenum)                
            if (port_dst != '-' and (int(port_dst) > 65535 or int(port_dst) < 1)):
                raise TypeError("Destination Port for rulenum %s is invalid" % rulenum)                
            if (protocol != '-' and (protocol not in ('T','B','U','I','O'))):
                raise TypeError("protocol for rulenum %s is invalid (seeing protocol %s)" % (rulenum,protocol))  
            if (ipproto != '-' and (int(ipproto) > 65535 or int(ipproto) < 1)):
                raise TypeError("Source Port for rulenum %s is invalid" % rulenum)  
